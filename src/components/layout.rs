...
pub fn Header() -> impl IntoView {
    ...
    view! {
        <header class="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-lg border-b border-gray-200/60">
            <div class="flex items-center justify-between px-4 py-3 max-w-lg mx-auto">
                <h1 class="text-lg font-bold text-gray-900">{move || i18n.t("app.title")}</h1>
                <button
...
pub fn BottomNav() -> impl IntoView {
    ...
    view! {
        <nav class="fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-lg border-t border-gray-200/60 safe-area-bottom">
            <div class="flex items-center justify-around max-w-lg mx-auto py-2">
                <a href="/" class=move || format!("flex flex-col items-center gap-0.5 px-3 py-1 {}", is_active("/"))>
                    ...
                    <span class="text-[10px]">{move || i18n.t("nav.dashboard")}</span>
                </a>
                <a href="/assets" class=move || format!("flex flex-col items-center gap-0.5 px-3 py-1 {}", is_active("/assets"))>
                    ...
                    <span class="text-[10px]">{move || i18n.t("nav.assets")}</span>
                </a>
                <a href="/register" class="flex flex-col items-center gap-0.5 px-3 py-1">
                    <div class="w-11 h-11 bg-gray-900 rounded-full flex items-center justify-center -mt-5 shadow-lg active:bg-gray-700 transition-colors">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                        </svg>
                    </div>
                    <span class="text-[10px] text-gray-400">{move || i18n.t("nav.register")}</span>
                </a>
                <a href="/settings" class=move || format!("flex flex-col items-center gap-0.5 px-3 py-1 {}", is_active("/settings"))>
                    ...
                    <span class="text-[10px]">{move || i18n.t("nav.settings")}</span>
                </a>
            </div>
        </nav>
    }
